...
掌握 Python 字典是成为熟练开发者的必经之路。为了帮助你记忆，这里有一份核心要点清单：

- **创建**：除了 `{}`，别忘了 `dict()` 和 `zip()` 的强大功能。

- **访问**：养成使用 `.get()` 的习惯，避免 `KeyError` 崩溃。

- **遍历**：`.items()` 是最常用的遍历方式，同时获取键值对。

- **修改**：`update()` 支持批量更新，原理是“有则覆盖，无则添加”。

- **复制**：处理嵌套数据时，务必使用 `copy.deepcopy()` 以避免数据污染。

- **技巧**：`fromkeys()` 用于初始化，`setdefault()` 用于智能赋值。

- **字典同其他数据结构之间的转换：**

  | 转换方向          | 核心函数/方法                        | 注意事项                       |
  | ----------------- | ------------------------------------ | ------------------------------ |
  | **字典 → 列表**   | `list(d.keys())`, `list(d.values())` | 默认转列表只取键               |
  | **列表 → 字典**   | `dict(list)`                         | 列表元素必须是 `[键, 值]` 形式 |
  | **字典 → 字符串** | `json.dumps(d)`                      | 推荐用于数据传输和存储         |
  | **字符串 → 字典** | `json.loads(s)`                      | 字符串必须是合法的 JSON 格式   |
  | **字典 → 集合**   | `set(d)`                             | 仅提取键，且顺序不固定         |